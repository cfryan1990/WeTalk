...
public class RosterProvider extends ContentProvider {
	...
	public static final String AUTHORITY = "com.hdu.cfryan.provider.Roster";
	public static final Uri CONTENT_URI = Uri.parse("content://" + AUTHORITY
			+ "/" + PreferenceConstants.TABLE_ROSTER);
	...
	static {
		URI_MATCHER.addURI(AUTHORITY, PreferenceConstants.TABLE_ROSTER, CONTACTS);
		URI_MATCHER.addURI(AUTHORITY, PreferenceConstants.TABLE_ROSTER + "/#", CONTACT_ID);
	}
	...
	public int delete(Uri url, String where, String[] whereArgs) {
		...
		case CONTACTS:
			count = db.delete(PreferenceConstants.TABLE_ROSTER, where, whereArgs);
			break;
	...
	public Uri insert(Uri url, ContentValues initialValues) {
		...
		long rowId = db.insert(PreferenceConstants.TABLE_ROSTER, RosterConstants.JID, values);
	...
	public Cursor query(Uri url, String[] projectionIn, String selection,
		...
		case GROUPS:
			qBuilder.setTables(PreferenceConstants.TABLE_ROSTER + " " + QUERY_ALIAS);
		...
		if (TextUtils.isEmpty(sortOrder) && match == CONTACTS) {
			orderBy = RosterConstants.DEFAULT_SORT_ORDER;		//Ĭ�ϰ�����״̬����		
			} else {
			orderBy = sortOrder;
		}
	...
	public int update(Uri url, ContentValues values, String where,
		...
		switch (match) {
		case CONTACTS:
			count = db.update(PreferenceConstants.TABLE_ROSTER, values, where, whereArgs);
			break;
		case CONTACT_ID:
			String segment = url.getPathSegments().get(1);
			rowId = Long.parseLong(segment);
			count = db.update(PreferenceConstants.TABLE_ROSTER, values, "_id=" + rowId, whereArgs);
			break;
		default:
