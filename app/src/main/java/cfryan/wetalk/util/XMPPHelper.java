...
public class XMPPHelper {
	...
	public static String splitJidAndResource(String account) {
		if (!account.contains("/"))
			return account;
		String[] res = account.split("/");
		String userName = res[0];
	...
	public static CharSequence convertNormalStringToSpannableString(
		...
		while (localMatcher.find()) {
			...
			if (m - k < 8) {
				if (Wanglai.getInstance().getFaceMap().containsKey(str2)) {
					int face = Wanglai.getInstance().getFaceMap().get(str2);
